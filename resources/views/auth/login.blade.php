<x-guest-layout>
    ...
    <form method="POST" action="{{ route('login') }}">
        ...
        <div>
            <x-input-label for="user_credentials" :value="__('Email Or Username')" />
            <x-text-input id="user_credentials" class="block mt-1 w-full" type="user_credentials" name="user_credentials"
                :value="old('user_credentials')" required autofocus autocomplete="username" />
            <x-input-error :messages="$errors->get('user_credentials')" class="mt-2" />
        </div>
